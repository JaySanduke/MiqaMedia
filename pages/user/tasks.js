...
export default function Tasks() {
  ...
  const [snapshot, loading, error] = useObject(
    ref(database, "users/" + uid + "/workspace/" + wpid)
  );
  ...
  function addTask(data) {
    if (uid) {
      const postk = push(
        ref(database, "users/" + uid + "/workspace/" + wpid + "/tasks/")
      ).key;
      ...
      update(
        ref(
          database,
          "users/" + uid + "/workspace/" + wpid + "/tasks/" + postk
        ),
        taskdetails
      );
    }
  ...
  function deleteTask(taskid) {
    ...
    if (taskid && taskid != "") {
      remove(
        ref(
          database,
          "users/" + uid + "/workspace/" + wpid + "/tasks/" + taskid
        )
      )
